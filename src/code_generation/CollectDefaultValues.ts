...
import { NODE_TYPES_WITH_DEFAULT_VALUES, DESTRUCTURING_PATTERN_NODE_TYPES } from '../constants'
...
export class CollectDefaultValues {
  ...
  rec = (node: Parser.SyntaxNode): string[] => {
    if (NODE_TYPES_WITH_DEFAULT_VALUES.includes(node.type))
      ...
      else if (DESTRUCTURING_PATTERN_NODE_TYPES.includes(node.namedChild(0).type))
