import { BuildType, TypeConstraint, TypeEqualityGraph } from '../../types/util'
import {
...
export class InferImportBindingTypes {
  ...
  perform = (node: SyntaxNode): void => {
    assert(node.type === 'import', 'Should be import node.')

    this.traverse(node)
  }

  private traverse = (node: SyntaxNode): void => {
    switch (node.type) {
  ...
  private handleIdentifier = (node: SyntaxNode): void => {
