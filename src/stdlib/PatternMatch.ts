...
export class PatternMatch {
  ...
  perform = (pattern: any, value: any, depth = 0): any[] => {
    ...
    else if (PatternMatch.matchesLiteral(pattern, value)) return []
  ...
  private isOmittable = (pattern: any): boolean => {
    const identifierPatternsCount = PatternMatch.identifierPatternsCount(
      pattern,
    )

    return (
      this.defaults.length >= identifierPatternsCount &&
      this.defaults
