...
import { SourceNodeWithInferrableType } from '../types/nodes'
...
export const inferTypesOfSource = (
  typedFileScopes: TypedFileScope[],
  ...
  return buildTypedSourceFileScope(
    finalFileScope,
    finalTypedScopes,
    answer.typedNode,
...
const buildPrimitiveAnswer = <T extends SourceNodeWithInferrableType>(
  state: State,
  node: T,
  type: PrimitiveType,
...
const traverseAll = <T extends SourceNodeWithInferrableType>(
  state: State,
  nodes: T[],
...
const ensureIsInstanceOf = <T extends SourceNodeWithInferrableType>(
...
const traverse = <T extends SourceNodeWithInferrableType>(
  state: State,
  node: T,
  context: Context,
): Answers<State, Return<T>> => {
...
const handleNode = (
  state: State,
  node: SourceNodeWithInferrableType,
  context: Context,
