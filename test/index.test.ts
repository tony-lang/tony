...
const runExample = (name: string, source: string): string => {
  const sourcePath = path.join(__dirname, `${name}.tn`)

  fs.writeFileSync(sourcePath, source)

  try {
    return childProcess.execSync(`tony run ${sourcePath}`).toString()
  } catch (error) {
